      ...
      <div class="radar-charts-box">
        ...
        >
          <div class="direction-left">
            ...
            <div v-for="item in radarIndicator" :key="item.name">
              {{ item.name }}
            </div>
          </div>
  ...
  methods: {
    ...
    async getData() {
      ...
      let {
        ...
        factArr,
        quotaObj
      } = this.getRadarChartsData(quotaList);
      //新增
      //获取数组最大值
      ...
      let { xAxisLine, yAxisLine, actualYData } = this.getLineChartsData(
        realisticList,
        quotaObj
      );
    ...
    getLineChartsData(data, quotaObj) {
      ...
      data.forEach(item => {
        ...
        let a = this._.divide(
          ((item.realisticValue * 100) / quotaObj[item.quotaKey]) * 100
        );
        a = this._.round(a, 2);
