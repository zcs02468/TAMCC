...
<template>
  <div class="panel left-container-angle">
    <div class="title" @click="SET_MAP_DIALOG_TYPE('electricity')">
      T1航站楼用电占比
    </div>
    <div class="content">
      <div class="select-box">
        <ul>
          <li
            :class="selectType == 'electric' ? 'select' : ''"
            @click="selectClick('electric')"
          >
            电力
          </li>
          <li
            :class="selectType == 'water' ? 'select' : ''"
            @click="selectClick('water')"
          >
            市政水
          </li>
          <li
            :class="selectType == 'gas' ? 'select' : ''"
            @click="selectClick('gas')"
          >
            天然气
          </li>
          <li></li>
  ...
  methods: {
    ...
    sortingData(data) {
      ...
      data.forEach((item, index) => {
        if (
          item.fengitemname == "总用电" ||
          item.fengitemname == "总用水" ||
          item.fengitemname == "总用气"
        ) {
          sum = item.fusedvalue;
        } else {
          arr.push({
